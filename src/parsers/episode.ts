...
const EpisodeSchema = z
  ...
  .transform<Episode>((data) => ({
    ...
    slug: data.slug,
    date: data.date,
    credits: data.credits?.credits.map((credit) => ({
      name: credit.name,
    ...
    season: data.season,
    bio_image: {
      large: applyImageBaseUrl(data.bio_image?.data?.attributes.formats?.large?.url),
      small: applyImageBaseUrl(data.bio_image?.data?.attributes.formats?.small?.url),
      medium: applyImageBaseUrl(data.bio_image?.data?.attributes.formats?.medium?.url),
      thumbnail: applyImageBaseUrl(data.bio_image?.data?.attributes.formats?.thumbnail?.url)
    },
    episode_image: {
      large: applyImageBaseUrl(data.episode_image?.image?.data?.attributes.formats.large?.url),
      small: applyImageBaseUrl(data.episode_image?.image?.data?.attributes.formats.small?.url),
      medium: applyImageBaseUrl(data.episode_image?.image?.data?.attributes.formats.medium?.url),
      thumbnail: applyImageBaseUrl(data.episode_image?.image?.data?.attributes.formats.thumbnail?.url)
    },
    guests: data.guests?.reduce<Episode['guests']>((acc, guest) => {
      if (guest.name) {
        acc.push({
          name: guest.name,
    ...
    links: data.links?.reduce<Episode['links']>((acc, link) => {
        if (link.link_url) {
          acc.push({
