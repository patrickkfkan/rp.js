**rp.js**

***

# rp.js

## Enumerations

- [AudioQuality](enumerations/AudioQuality.md)

## Classes

- [Player](classes/Player.md)
- [RadioParadise](classes/RadioParadise.md)

## Interfaces

- [AlbumInfo](interfaces/AlbumInfo.md)
- [ArtistInfo](interfaces/ArtistInfo.md)
- [Channel](interfaces/Channel.md)
- [GetAlbumInfoParams](interfaces/GetAlbumInfoParams.md)
- [GetArtistInfoParams](interfaces/GetArtistInfoParams.md)
- [GetNowPlayingListParams](interfaces/GetNowPlayingListParams.md)
- [GetSongInfoParams](interfaces/GetSongInfoParams.md)
- [Logger](interfaces/Logger.md)
- [NowPlayingList](interfaces/NowPlayingList.md)
- [PlayerStatus](interfaces/PlayerStatus.md)
- [RadioParadiseOptions](interfaces/RadioParadiseOptions.md)
- [SongInfo](interfaces/SongInfo.md)
- [Track](interfaces/Track.md)

## Type Aliases

- [BlockTrack](type-aliases/BlockTrack.md)
- [PlayerTrackInfo](type-aliases/PlayerTrackInfo.md)

## Functions

- [millisToLocaleDateTimeString](functions/millisToLocaleDateTimeString.md)
- [millisToMinutesAndSeconds](functions/millisToMinutesAndSeconds.md)
